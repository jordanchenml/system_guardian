...
async def send_event_to_api(session: aiohttp.ClientSession, event: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        logger.info(f"Sending {event['source']} {event['event_type']} event to {url}")
        ...
        if response.status in (200, 201, 202):
            ...
            response_text = await response.text()
            logger.error(f"Error sending event to API: {response.status}")
...
async def create_similar_incident_group(session: aiohttp.ClientSession, group_index: int) -> List[str]:
    """
...
async def test_incident_detection(session: aiohttp.ClientSession):
    ...
    for source in EVENT_SOURCES:
        for category in random.sample(list(ENHANCED_TEMPLATES.keys()), min(2, len(ENHANCED_TEMPLATES))):
            event_type = random.choice(EVENT_SOURCES[source])
...
async def test_severity_classification(session: aiohttp.ClientSession):
    ...
    for severity in ["critical", "high", "medium", "low"]:
        ...
        if matching_templates:
            ...
            logger.info(f"Creating {severity} severity incident from {source} {event_type}")
...
async def generate_and_send_events(
    ...
    async with aiohttp.ClientSession() as session:
        if specific_test == "detection":
            ...
            logger.info(f"Generating {num_events} random incidents...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate enhanced test events for System Guardian")
    parser.add_argument("--url", type=str, help=f"Base URL of the API (default: {BASE_URL})")
    parser.add_argument("--events", type=int, default=20, help="Number of random events to generate")
    parser.add_argument("--test", type=str, choices=["detection", "severity", "similarity", "resolution", "all"], 
                        help="Specific test to run")
    parser.add_argument("--similar-group", type=int, help="Specific similar incident group to create (0-2)")
    ...
    if args.test:
        ...
        logger.info(f"Generating {args.events} random incidents")
