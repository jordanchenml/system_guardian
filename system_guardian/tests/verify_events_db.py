...
async def verify_recent_events(minutes=10, limit=20):
    ...
    """
    logger.info(f"Checking for events in the database from the last {minutes} minutes...")
    ...
    try:
        async with session_factory() as session:
            ...
            if recent_count == 0:
                ...
                if total_count == 0:
                    logger.warning("The events table is empty. No events have been stored.")
                else:
                    ...
                    if most_recent:
                        logger.info(f"Most recent event: ID={most_recent[0]}, Source={most_recent[1]}, Type={most_recent[2]}")
                        logger.info(f"Created at: {most_recent[3]} ({datetime.now() - most_recent[3]} ago)")
            ...
            for i, event in enumerate(recent_events):
                ...
                logger.info(f"Created: {created_at} ({datetime.now() - created_at} ago)")
                ...
                try:
                    ...
                    if isinstance(content, dict) and 'raw_payload' in content:
                        if isinstance(content['raw_payload'], dict) and 'test_id' in content['raw_payload']:
            ...
            if test_events:
                ...
                for event in test_events:
                    ...
                    try:
                        ...
                        if isinstance(content, dict) and 'raw_payload' in content:
                            if isinstance(content['raw_payload'], dict) and 'test_id' in content['raw_payload']:
                                logger.info(f"Test ID: {content['raw_payload']['test_id']}")
                    except:
                        pass
