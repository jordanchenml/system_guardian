...
EVENT_SOURCES = {
    ...
    "jira": ["issue_created", "issue_updated", "issue_resolved", "issue_commented", "sprint_started"],
        ...
        "issue": [
            {
                ...
                "keywords": ["error", "api", "500", "endpoint", "critical", "broken", "failure"],
            ...
            },
            {
                ...
                "keywords": ["database", "failure", "connection", "emergency", "urgent"],
        ...
        "pull_request": [
            {
                ...
                "keywords": ["database", "connection", "pool", "fix", "hotfix", "urgent"],
            ...
            },
            {
                ...
                "keywords": ["optimize", "image", "processing", "memory", "emergency", "fix", "critical"],
    ...
    "jira": {
        "issue_created": [
            {
                ...
                "keywords": ["database", "latency", "production", "blocker", "critical"],
            ...
            },
            {
                ...
                "keywords": ["authentication", "login", "intermittent", "credentials", "critical", "blocker", "incident"],
            ...
            },
            {
                ...
                "keywords": ["database", "replication", "lag", "urgent", "inconsistency"],
            ...
            },
            {
                ...
                "keywords": ["kubernetes", "nodes", "down", "production", "critical", "outage"],
        ...
        "issue_resolved": [
            ...
            },
            {
                ...
                "keywords": ["database", "performance", "critical", "problem", "outage", "incident"],
    ...
    "datadog": {
        "alert": [
            ...
            },
            {
                ...
                "keywords": ["error", "payment", "processing", "rate", "incident", "critical", "failure"],
            ...
            },
            {
                ...
                "keywords": ["database", "connection", "outage", "failures", "critical", "emergency"],
        ...
        "metric_alert": [
            {
                ...
                "keywords": ["memory", "database", "threshold", "exceeded", "urgent", "critical", "outage"],
...
async def generate_event_with_llm(source: str, event_type: str, severity: str = None) -> Dict[str, Any]:
    """
    ...
    if not openai_client:
        ...
        return random.choice(ISSUE_TEMPLATES.get(source, {}).get(event_type, [{"title": f"Default {source} {event_type} event", "description": "Default description", "keywords": ["default"]}]))
...
def generate_raw_payload(
    ...
    if source == "github":
        ...
        if "issue" in event_type:
            ...
            if random.random() < 0.7:  # 70% chance to use actions that may trigger events
                action = random.choice(possible_actions)
                
        # Ensure description has enough keyword information
        if source_keywords and random.random() < 0.8:  # 80% chance to strengthen keywords
        ...
        if event_type == "push":
            ...
            author = {
                "name": random.choice(["jordanchenml", "dev_user", "admin_user", "test_user"]),
                "email": f"{random.choice(['jordan', 'dev', 'admin', 'test'])}@example.com",
                "username": random.choice(["jordanchenml", "dev_user", "admin_user", "test_user"])
            }
            ...
            commits = [{
                ...
                "added": [f"system_guardian/{random.choice(['services', 'web', 'db'])}/{random.choice(['file1.py', 'file2.py', 'file3.py'])}"],
                "author": author,
                "message": title,
                "removed": [],
                "tree_id": "".join(random.choice("0123456789abcdef") for _ in range(40)),
                "distinct": True,
                "modified": [f"system_guardian/{random.choice(['services', 'web', 'db'])}/{random.choice(['existing1.py', 'existing2.py', 'existing3.py'])}"],
                "committer": author,
            ...
            issue = {
                ...
                "created_at": (datetime.datetime.now() - datetime.timedelta(days=random.randint(1, 30))).strftime("%Y-%m-%dT%H:%M:%SZ"),
                "updated_at": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
                "closed_at": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ") if status == "closed" else None,
                "body": description,
            ...
            for label_name in label_names:
                label_color = "".join(random.choice("0123456789abcdef") for _ in range(6))
                issue["labels"].append({
            ...
            merged = status == "merged" or (status == "closed" and random.random() < 0.8)
            pull_request = {
                "id": pr_id,
                ...
                "created_at": (datetime.datetime.now() - datetime.timedelta(days=random.randint(1, 30))).strftime("%Y-%m-%dT%H:%M:%SZ"),
                "updated_at": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
                "closed_at": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ") if status in ["closed", "merged"] else None,
                "merged_at": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ") if merged else None,
                "merge_commit_sha": "".join(random.choice("0123456789abcdef") for _ in range(40)) if merged else None,
                "head": {
                    "ref": random.choice(["feature-branch", "fix-branch", "improvement-branch"]),
            ...
            label_names = [severity]
            for keyword in keywords:
                if keyword in ["fix", "hotfix", "urgent", "emergency"] and random.random() < 0.7:  # 70% chance to add keyword label
                    label_names.append(keyword)
        ...
        if source_keywords and random.random() < 0.8:
            ...
            if not selected_keywords:
                selected_keywords = random.sample(source_keywords, min(2, len(source_keywords)))
                
            for keyword in selected_keywords:
                if keyword.upper() not in jira_description:
                    jira_description += f" This is marked as {keyword.upper()} priority."
        ...
        payload = {
            "user": {
                ...
                "displayName": random.choice(["John Doe", "Jane Smith", "Alex Johnson", "Sam Taylor", "Terry Kim"])
            },
            "issue": {
                "id": issue_id,
                "key": issue_key,
                ...
                "fields": {
                    ...
                    "created": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "+0800",
                    "creator": {
                        "self": f"https://example.atlassian.net/rest/api/2/user?accountId=user123456",
                        "active": True,
                        "timeZone": "Asia/Taipei",
                    ...
                    "summary": title,
                    "updated": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "+0800",
                    "watches": {
            ...
            "changelog": {
                ...
                "items": [
                    ...
                    },
                    {
                        ...
                        "field": "reporter",
                        "fieldId": "reporter",
                        "toString": random.choice(["John Doe", "Jane Smith", "Alex Johnson", "Sam Taylor", "Terry Kim"]),
                        "fieldtype": "jira",
                        "fromString": None,
        ...
        payload.update({
            ...
            "priority": severity,
            "tags": tags,
            "hostname": random.choice(["app-server-01", "db-server-02", "api-server-03", "worker-04", "cache-05"]),
...
async def send_event_to_api(session: aiohttp.ClientSession, event: Dict[str, Any]) -> bool:
    ...
    try:
        async with session.post(endpoint, json=api_payload, headers=headers) as response:
...
async def create_incident_directly(session: aiohttp.ClientSession, event: Dict[str, Any]) -> Optional[int]:
    """
...
async def add_resolution_to_incident(session: aiohttp.ClientSession, incident_id: int, event: Dict[str, Any]) -> bool:
    """
    ...
    resolution_payload = {
        ...
        "resolved_by": random.choice(["John Doe", "Jane Smith", "Alex Johnson", "Sam Taylor", "Terry Kim"]),
...
def main():
    ...
    :rtype: None
    """
    parser = argparse.ArgumentParser(description="Generate fake events for System Guardian")
    parser.add_argument("--events", type=int, default=50, help="Number of events to generate")
    parser.add_argument("--days", type=int, default=60, help="Maximum days in the past to generate events")
