...
def configure_loguru(
    ...
    def filter_noisy_loggers(record):
        # 完全禁用這些命名空間的日誌
        blocked_loggers = ["sqlalchemy", "aiokafka", "aio_pika", "asyncio", 
                          "httpx", "urllib3", "uvicorn.access"]
        ...
        # 過濾掉警告訊息中的特定內容
        if "Qdrant client version" in record["message"] and "incompatible with server version" in record["message"]:
            return False
...
def configure_sqlalchemy_logging():
    ...
    for name in ['sqlalchemy', 'sqlalchemy.engine', 'sqlalchemy.pool', 'sqlalchemy.orm', 'sqlalchemy.dialects']:
        # 完全禁用這些記錄器
...
def configure_logging():
    ...
    for logger_name in ["sqlalchemy", "aiokafka", "aio_pika", "asyncio", "httpx", 
                       "urllib3", "uvicorn.access", "fastapi"]:
...
class InterceptHandler(logging.Handler):
    ...
    def intercept_all_loggers(self):
        ...
        for unwanted_logger in ["sqlalchemy", "aiokafka", "aio_pika", "asyncio"]:
        ...
        for logger_name in logging.root.manager.loggerDict:
            # 跳過不想處理的日誌
            if any(name in logger_name for name in ["sqlalchemy", "aiokafka", "aio_pika", "asyncio"]):
                continue
    ...
    def _intercept_logger(self, logger_name: str):
        ...
        # 完全禁用不想要的日誌
        if any(name in logger_name for name in ["sqlalchemy", "aiokafka", "aio_pika", "asyncio", "httpx", "urllib3"]):
    ...
    def emit(self, record):
        ...
        """
        # 檢查記錄是否應該被完全忽略
        if any(name in record.name.lower() for name in ["sqlalchemy", "aiokafka", "aio_pika", "httpx", "asyncio", "urllib3"]):
            return
        ...
        if record.getMessage() and (
            "raw sql" in record.getMessage() or 
            "generated in" in record.getMessage() or 
            "cached since" in record.getMessage() or
            "HTTP Request" in record.getMessage() or
            "Qdrant client version" in record.getMessage()
        ):
            return
        ...
        if any(prefix in record.name for prefix in ['uvicorn', 'fastapi']) and record.levelno < logging.WARNING:
            return
