        ...
        async with self.db_session_factory() as session:
            ...
            if resolution:
                resolution_data = {
                    ...
                    "generated_at": resolution.generated_at.isoformat() if resolution.generated_at else None
                }
            
            # Generate the report using AI
            try:
                # Use specialized model for report generation
                model = settings.ai_report_generation_model if settings.ai_allow_advanced_models else settings.openai_completion_model
                ...
                response_format = {"type": "json_object"} if format == ReportFormat.JSON else None
    ...
    async def generate_operational_recommendations(
        ...
        async with self.db_session_factory() as session:
            ...
            for resolution, incident in resolutions_with_incidents:
                if resolution.suggestion:
                    resolution_data = {
                        ...
                        "suggestion": resolution.suggestion[:300] + "..." if len(resolution.suggestion) > 300 else resolution.suggestion,
                        "is_applied": resolution.is_applied
                    }
            ...
            try:
                ...
                model = settings.ai_resolution_generation_model if settings.ai_allow_advanced_models else settings.openai_completion_model
