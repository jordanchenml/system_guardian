...
async def create_kafka_producer():
    ...
    producer = AIOKafkaProducer(
        ...
        client_id="system_guardian_event_consumer"
    )
...
async def main():
    ...
    try:
        async with session_factory() as test_session:
            ...
            logger.info(f"Database session test successful! Current database time: {timestamp}")
    ...
    try:
        ...
        logger.warning("Continuing to start service, but AI functionality may be unavailable")
        ai_engine = None
    
    # 創建和初始化Kafka生產者和RabbitMQ通道池
    ...
    for sig in signals:
        loop.add_signal_handler(sig, lambda: asyncio.create_task(shutdown(consumer, engine, kafka_producer)))
    ...
    try:
        ...
        await shutdown(consumer, engine, kafka_producer)
    
    logger.info("Event consumer service terminated")


async def shutdown(consumer, engine, kafka_producer):
